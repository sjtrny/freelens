...
OTHER_MESSAGE = "101100010010011010011100"
...
def test_colour_correction_leaves_unusable_channel_on_rgb_scale():
    ...
    np.testing.assert_allclose(
        corrected, np.array([[[64 / 255, 100 / 255, 192 / 255]]])
    )


def test_decode_frames_uses_corrected_frame(monkeypatch):
    image = Tag.from_message(MESSAGE).to_image(quiet_pad_size=0)
    corrected = (
        np.array(
            Tag.from_message(OTHER_MESSAGE).to_image(quiet_pad_size=0),
            dtype=np.float32,
        )
        / 255
    )
    ...
    assert [tag.message for tag in tags] == [OTHER_MESSAGE]


def test_decode_frames_keeps_original_when_correction_collapses_palette(monkeypatch):
    image = Tag.from_message(MESSAGE).to_image(quiet_pad_size=0)
    monkeypatch.setattr(
        freelens,
        "_correct_colours",
        lambda image, black, white: np.zeros_like(image, dtype=np.float32),
    )

...
To ensure data integrity, a 5×5 grid carries a 16-bit CRC in its center row and
column, excluding the center cell. Freelens validates the deployed calculation
over all 16 non-cross cells, including the four corners, in column-major order.
The patent-derived message-only implementation has been removed because it does
not reproduce observed deployed 5×5 tags. The complete tuple and reproducible
known answers are documented in [CRCs in ddTags](docs/ddtag-crc.md).
